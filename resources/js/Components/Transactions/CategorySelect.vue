<template>
  <div class="relative">
    ...
    <div
      v-if="showResults"
      class="absolute z-50 max-h-60 w-96 overflow-y-scroll rounded-md border border-gray-300 bg-white shadow-sm"
    >
      <div
        v-for="(category, categoryIdx) in filteredCategories"
        :key="category.id"
        class="group flex cursor-pointer items-center p-2 hover:bg-blue-600 hover:text-white"
        :class="{
