<template>
  <div class="col-span-8 divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow">
    <div class="px-4 py-5 font-sans font-semibold text-astronaut-500 sm:px-6">Current Budget</div>
    <div class="px-4 pt-5 sm:p-6">
      <table v-if="budgetsPeriods.length > 0" class="min-w-full divide-y divide-gray-300">
        <thead>
          <tr>
            <th
              scope="col"
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="budgetPeriod in budgetsPeriods" :key="'budget-period-' + budgetPeriod.id">
            ...
            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500">
              {{ budgetPeriod.category.name }}
            </td>
            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500">
              {{ formatMoney(budgetPeriod.amount) }}
            </td>
            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500">
              {{ findCurrentAmount(budgetPeriod) }}
            </td>
            <td class="whitespace-nowrap py-4 px-3 text-sm">
              <span
                class="rounded-full px-2 py-1"
                :class="{
                  'bg-green-100 text-green-800': findRemainingAmount(budgetPeriod) > 0,
                  'bg-red-100 text-red-800': findRemainingAmount(budgetPeriod) < 0,
                  'bg-gray-100 text-gray-900': findRemainingAmount(budgetPeriod) == 0,
                }"
...
function loadBudgetsPeriods() {
  BudgetsPeriodsAPI.index().then((response) => {
...
function createBudgetPeriods() {
  BudgetsPeriodsAPI.store().then((response) => {
    loadBudgetsPeriods();
  });
}

function findCurrentAmount(budgetPeriod) {
  let currentSpent = 0.0;
