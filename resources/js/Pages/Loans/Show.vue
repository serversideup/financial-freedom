  ...
  <app-layout>
    <div v-if="loaded">
      <div class="mt-2 md:flex md:items-center md:justify-between">
        <div class="min-w-0 flex-1">
          ...
          >
            <img :src="loan.institution.logo" class="mr-2 h-8 w-8 rounded-full" />
            {{ loan.name }}
          </h2>
        </div>
    ...
    <div v-if="loaded" class="mt-5">
      <stats
        :account="loan"
        :type="'loan'"
        :fields="['current_balance', 'interest_rate', 'payment_amount']"
      />
...
export default {
  ...
  methods: {
    ...
    loadLoan() {
      LoansAPI.show(this.id).then((response) => {
