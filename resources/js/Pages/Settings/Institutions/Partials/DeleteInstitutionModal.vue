    ...
    <Modal @close="closeModal" :show="show" :max-width="'xl'">
        <div class="flex flex-col w-full">
            <div class="flex w-full">
                ...
                <div class="flex-1 ml-4">
                    <div class="flex items-center justify-between w-full">
                        <span class="font-sans text-lg font-semibold text-[#F5F5F6]">Delete Institution</span>
                        <button @click="closeModal">
                            <ModalCloseIcon/>
                        </button>
                    </div>
                    <span class="font-sans text-sm text-[#94969C]">Are you sure you want to delete this institution?</span>
                </div>
            </div>
            <div class="pt-8 flex items-center justify-end">
                ...
                <button @click="deleteInstitution" class="ml-3 bg-[#D92D20] text-white cursor-pointer px-4 py-[10px] rounded-lg font-semibold border border-[#D92D20]">
                    Delete
                </button>
            </div>
        </div>
...
const listener = ( event, data ) => {
    if( event == 'prompt-delete-institution' ){
...
const deleteInstitution = () => {
    router.delete('/settings/institutions/'+institution.value.id, {
        preserveScroll: true,
        onSuccess: () => {
            closeModal();
            notifyBus.emit('notify', {
                title: 'Institution Deleted',
                body: 'The institution has been deleted.',
                type: 'success'
            });
        }
    });
