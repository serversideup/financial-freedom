<template>
  <app-layout>
    ...
    <h3 class="mt-5 text-lg font-medium leading-4 text-gray-900">
      {{ dateRange }}
    </h3>
      ...
      <div class="flex w-full flex-col">
        ...
        >
          <template #buttons>
            <button
              type="button"
              class="focus:shadow-outline-blue relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-4 py-3 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out hover:text-gray-500 focus:z-10 focus:border-blue-300 focus:outline-none active:bg-gray-100 active:text-gray-700"
              @click="download()"
            >
  ...
  computed: {
    ...
    dateRange() {
      if (this.startDate && this.endDate) {
        return `${moment(this.startDate).format('LL')} - ${moment(
